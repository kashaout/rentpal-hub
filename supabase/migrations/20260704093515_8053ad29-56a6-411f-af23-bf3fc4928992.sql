...
BEGIN
  -- 0) tenants bridge (idempotent) — CRITICAL, must not be swallowed
  ...
  IF _tenant_bridge_id IS NULL THEN
    RAISE EXCEPTION 'activate_lease_lifecycle: failed to resolve tenants bridge row for lease %', NEW.id;
  END IF;

  -- 0b) payments bridge (idempotent) — CRITICAL
  IF NOT EXISTS (
    SELECT 1 FROM payments
    WHERE tenant_id = _tenant_bridge_id
      AND lease_id = NEW.id
  ) THEN
    INSERT INTO payments (
      ...
      _tenant_bridge_id, NEW.id, NEW.property_id,
      COALESCE(NEW.rent_amount, 0), 'pending', NEW.lease_start
    );
  END IF;

  -- A) booking creation (idempotent) — CRITICAL
  ...
  -- C) lease_credentials (idempotent) — CRITICAL
  ...
  -- D) landlord notification — NON-CRITICAL, log-and-continue
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'activate_lease_lifecycle: landlord notification failed for lease %: %', NEW.id, SQLERRM;
  END;
